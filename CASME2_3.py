...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, num_loso, transform = None, basic_aug = False, transform_norm=None):
        ...
        df = pd.read_excel(os.path.join('/data2/wangzihan/data_process/', 'CASME2-coding-20190701.xlsx'),usecols=[0,1,3,4,5,7,8])
        df['Subject'] = df['Subject'].apply(str)

        if phase == 'train':
            dataset = df.loc[df['Subject']!=num_loso]   #pandas中的df.loc[]主要是根据DataFrame的行标和列标进行数据的筛选的
        else:
    ...
    def __getitem__(self, idx):
        ...
        path_on0 = os.path.join(self.raf_path, 'Cropped', num+sub, f, on0)
        path_on1 = os.path.join(self.raf_path, 'Cropped', num+sub, f, on1)
        path_on2 = os.path.join(self.raf_path, 'Cropped', num+sub, f, on2)
        path_on3 = os.path.join(self.raf_path, 'Cropped', num+sub, f, on3)
        path_apex0 = os.path.join(self.raf_path, 'Cropped', num+sub, f, apex0)
        path_off0 = os.path.join(self.raf_path, 'Cropped', num+sub, f, off0)
        path_off1 = os.path.join(self.raf_path, 'Cropped', num+sub, f, off1)
        path_off2 = os.path.join(self.raf_path, 'Cropped', num+sub, f, off2)
        path_off3 = os.path.join(self.raf_path, 'Cropped', num+sub, f, off3)
        ...
        if self.transform is not None:
            ...
            ALL = torch.cat((image_on0, image_apex0), dim=0)
        ...
        return image_on0, image_apex0, label_all, temp
...
class MMNet(nn.Module):
    def __init__(self):
        ...
        self.main_branch =resnet18_pos_attention(class_num=3)
...
def run_training():
    ...
    for subj in LOSO:
        ...
        if args.optimizer == 'adam':
            ...
            optimizer = torch.optim.SGD(params_all, args.lr,
        ...
        for i in range(1, 100):
            ...
            for batch_i, (image_on0, image_apex0, label_all, label_au) in enumerate(tqdm(train_loader)):
                iter_cnt += 1
            ...
            with torch.no_grad():
                ...
                net_all.eval()
                # net_au.eval()
                for batch_i, (image_on0,  image_apex0, label_all, label_au) in enumerate(val_loader):
