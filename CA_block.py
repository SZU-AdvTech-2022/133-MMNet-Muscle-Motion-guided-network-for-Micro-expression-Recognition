...
class ResNet(nn.Module):
    ...
    def __init__(self,class_num, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.fc = nn.Linear(512* block.expansion*196, self.class_num)
...
def _resnet(class_num, arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(class_num, block, layers, **kwargs)
    if pretrained:
...
def resnet18_pos_attention(class_num, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet(class_num, 'resnet18', CABlock, [1, 1, 1, 1], pretrained, progress,
                   **kwargs)
